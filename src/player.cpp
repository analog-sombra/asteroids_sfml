...
Player::Player()
    : Entity(sf::Vector2f(500, 500), 0), shape(sf::PrimitiveType::TriangleFan, 4),
      shootTimer(0.f)
{
...
void Player::update(float deltaTime)
{
    ...
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Space)  && shootTimer <= 0.f)
    {
        ...
        // Fire bullet
        toAddList.push_back(std::make_unique<Bullet>(bulletPos, sf::Vector2f(cos(radians), sin(radians))));
    }
}
